class App {
  ...
  loadEventListeners = () => {
    ...
    document.querySelector(this.ui.selectors.editNodeSubmitBtn).addEventListener('click', () => this.ui.editNodeSubmitHandler());
    document.querySelector(this.ui.selectors.addObstacleSubmitBtn).addEventListener('click', () => this.ui.addObstacleSubmitHandler());
    document.querySelector(this.ui.selectors.editObstacleSubmitBtn).addEventListener('click', () => this.ui.editObstacleSubmitHandler());
  ...
  // edit
  editNode = (id, label, obstacle) => {
    const currentNode = this.currentGraph.nodes.find(node => node.id === id);
    if (label)
      currentNode.label = label;

    currentNode.obstacle = obstacle;
    if (obstacle === '1') {
      this.graph.addObstacleNode(id);
    } else {
      this.graph.removeObstacleNode(id);
      // add original edge
      for (const node of this.currentGraph.edges) {
        if (node.start == id || node.end == id) {
          this.graph.addEdge(node.start, node.end, node.weight);
        }
      }
    }
  ...
  addNode = (x, y, label, obstacle) => {
    ...
    this.graph.addNode(id, obstacle);
  ...
  initSelects = () => {
    ...
    selects.forEach(select => {
      ...
      for (let i = 0; i < this.currentGraph.nodes.length; i++) {
        if (this.currentGraph.nodes[i].obstacle !== '1') {
  ...
  loadExampleGraph = () => {
    ...
    // Copy example graph:
    let randomGraphIdx = this.exampleGraphIdx == 0 ? 1 : 0;
    ...
    for (let i = 0; i < nodes.length; i++) {
      this.graph.addNode(nodes[i].id, nodes[i].obstacle);
    }
