class UICtrl {
  constructor() {
    this.selectors = {
      ...
      addObstacleSubmitBtn: '#add-obstacle-submit-btn',

      editNodeModal: '#edit-node-modal',
      editNodeLabel: '#edit-node-label',
      editNodeSubmitBtn: '#edit-node-submit-btn',
      editObstacleSubmitBtn: '#edit-obstacle-submit-btn',
  ...
  init = () => {
    ...
    M.Modal.init(document.querySelector(this.selectors.editNodeModal), {});
  ...
  addNodeSubmitHandler = () => {
    ...
    app.addNode(this.addNodeX, this.addNodeY, label, '0');
    this.addNodeX = null;
    this.addNodeY = null;
  }

  addObstacleSubmitHandler = () => {
    const labelInput = document.querySelector(this.selectors.addNodeLabel);
    const label = labelInput.value.trim();
    if (label === '') return;
    if (app.checkIfNodeExists(label)) return;
    ...
    // obstacle
    app.addNode(this.addNodeX, this.addNodeY, label, '1');
    this.addNodeX = null;
    this.addNodeY = null;
  }

  // edit
  editNodeSubmitHandler = () => {
    const labelInput = document.querySelector(this.selectors.editNodeLabel);
    const label = labelInput.value.trim();
    if (app.checkIfNodeExists(label)) return;
    ...
    M.Modal.getInstance(document.querySelector(this.selectors.editNodeModal)).close();

    app.editNode(this.editNodeId, label, '0');
    this.editNodeId = null;
  }

  // edit
  editObstacleSubmitHandler = () => {
    const labelInput = document.querySelector(this.selectors.editNodeLabel);
    const label = labelInput.value.trim();
    if (app.checkIfNodeExists(label)) return;
    ...
    // obstacle
    app.editNode(this.editNodeId, label, '1');
    this.editNodeId = null;
  }
  ...
  graphClickHandler = (e) => {
    // edit
    if (e.target.parentNode.classList.contains('graph-node') && app.currentState === APP_STATES.NODE_EDIT) {
      this.editNodeId = parseInt(e.target.parentNode.dataset.nodeId);

      M.Modal.getInstance(document.querySelector(this.selectors.editNodeModal)).open();
      document.querySelector(this.selectors.editNodeLabel).focus();
    }
